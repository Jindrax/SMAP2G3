...
public class CustomerProducer extends CommandProvider<CustomerAgent> {
    ...
    public void run() {
        while (true) {
            Localities locality = Randomizer.randomEnum(Localities.class);
            ...
            if (agent != null) {
                CustomerOrder order = new CustomerOrder(locality, agent.getAlias(), Randomizer.randomEnum(Catalog.class), Randomizer.randomInt(Configuration.getInt("MAX_QUANTITY_ORDER")));
